...
const Technologies = () => {
  return (
    ...
    >
      {/* Capa con imagen de fondo y opacidad */}
      <div
        className="absolute inset-0 bg-cover bg-center"
        style={{
          backgroundImage: `url(${img})`,
          opacity: 0.05, // Ajusta la opacidad de la imagen
        }}
      ></div>

      {/* Capa con otros fondos */}
      ...
      <div className="mx-auto relative">
        ...
        <div className="space-y-12 max-w-6xl mx-auto">
          ...
          <div>
            ...
            <div className="grid m-auto grid-cols-2 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-8 gap-3">
              {technologies.mastered.map((tech, index) => (
                <motion.div
                  ...
                  viewport={{ once: true }}
                  className="glass-card bg-white/5 backdrop-blur-sm p-3 text-center hover:scale-105 transition-transform group"
                >
                  <span
                    className={`text-5xl block group-hover:scale-110 transition-transform ${tech.color} group-hover:opacity-100 opacity-100`}
                  >
                    {tech.icon}
                  </span>
                  <h3 className="text-md font-medium mb-2">{tech.name}</h3>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-8 gap-3">
              {technologies.learning.map((tech, index) => (
                ...
                >
                  <span
                    className={`text-5xl block group-hover:scale-110 transition-transform ${tech.color} group-hover:opacity-100 opacity-80`}
                  >
                    {tech.icon}
                  </span>
                  <h3 className="text-md font-medium mb-2">{tech.name}</h3>
